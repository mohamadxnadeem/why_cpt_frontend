  ...
  useEffect(() => {
    ...
    if (cached) {
      try {
        ...
        // ✅ Sort cached data too (low → high)
        const sortedCached = [...parsed].sort((a, b) => {
          const priceA = Number(a?.car?.price || 0);
          const priceB = Number(b?.car?.price || 0);
          return priceA - priceB;
        });

        setData(sortedCached);
      } catch (err) {}
    }

    fetch("https://web-production-1ab9.up.railway.app/api/cars-for-hire/budget/", {
      cache: "no-store",
    })
      ...
      .then((newData) => {
        ...
        setData(sortedUpdated);
        localStorage.setItem(cacheKey, JSON.stringify(sortedUpdated));
      })
        ...
        <section className="tf-explore-2 tf-section live-auctions">
          <div className="themesflat-container">
            ...
            <p className="sub-title ct small mg-bt-20 pad-420">
              With professional driver (tour guide) to chauffeur you around the
              best spots in and around Cape Town
            </p>
                  ...
                  <SwiperSlide key={index}>
                    <div className="sc-card-product">
                      ...
                      >
                        ...
                        >
                          R{Math.round(item.car?.price || 0)} / Per Day
                        </span>
                      </div>
